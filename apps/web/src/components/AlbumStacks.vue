...
function albumPhotos(album: AlbumDetail): GalleryPhoto[] { return album.photos.map(toGalleryPhoto); }
...
<template>
  <section class="album-stacks" :aria-label="t('albums.photoAlbums')">
    ...
    <section v-else class="album-expanded" :aria-label="expandedAlbum.title" @click="collapseOnBlank">
      ...
      <div class="album-spread">
        <button v-for="(photo, index) in albumPhotos(expandedAlbum)" :key="photo.id" class="album-photo" type="button" :style="{ '--photo-delay': `${index * 55}ms`, '--photo-ratio': photo.aspectRatio, backgroundColor: photo.tone }" :aria-label="t('photo.open', { title: photo.title })" @click.stop="emit('open-photo', photo)">
...
.album-expanded { animation: expanded-in .34s ease both; min-height: 64vh; padding: 21px 0 86px; }.album-expanded-title { align-items: baseline; display: flex; gap: 16px; justify-content: space-between; padding-bottom: 25px; }.album-spread { display: grid; gap: 13px; grid-template-columns: repeat(3, minmax(0, 1fr)); width: 50%; }
.album-photo { animation: photo-spread-in .56s cubic-bezier(.2,.75,.25,1) both; animation-delay: var(--photo-delay); aspect-ratio: var(--photo-ratio); display: block; overflow: hidden; padding: 0; transform-origin: center center; }.album-photo img { display: block; height: 100%; object-fit: cover; transition: transform .45s ease; width: 100%; }.album-photo:hover img, .album-photo:focus-visible img { transform: scale(1.025); }
...
@media (max-width: 960px) { .album-stack-grid { gap: 38px 24px; grid-template-columns: repeat(auto-fill, minmax(190px, 220px)); }.album-spread { width: 70%; } }
@media (max-width: 580px) { .album-stack-grid { gap: 34px 17px; grid-template-columns: repeat(2, minmax(0, 1fr)); padding: 24px 0 68px 16px; }.album-stack-card { width: 100%; }.stack-photo { transform: translate(calc(var(--stack-index) * -7px), calc(var(--stack-index) * 6px)) rotate(calc(var(--stack-index) * -1.3deg)); }.album-stack:hover .stack-photo, .album-stack:focus-visible .stack-photo { transform: translate(calc(var(--stack-index) * -10px), calc(var(--stack-index) * 8px)) rotate(calc(var(--stack-index) * -1.7deg)); }.album-stack-copy { gap: 8px; padding-top: 33px; }.album-stack-copy h2, .album-expanded-title h2 { font-size: 17px; }.album-expanded { padding-top: 10px; }.album-spread { gap: 6px; grid-template-columns: repeat(2, minmax(0, 1fr)); width: 100%; } }
