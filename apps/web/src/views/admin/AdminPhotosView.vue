...
import { Check, ChevronDown, Copy, Eye, ImagePlus, MapPin, Star, Trash2, X } from 'lucide-vue-next';
...
function isBatchExpanded(group: AdminPhotoGroup): boolean { return expandedBatchKeys.value.has(group.key); }
function toggleBatchExpanded(group: AdminPhotoGroup): void {
  const next = new Set(expandedBatchKeys.value);
  if (next.has(group.key)) next.delete(group.key);
  else next.add(group.key);
  expandedBatchKeys.value = next;
}
function isBatchSelected(group: AdminPhotoGroup): boolean { return areAllBatchPhotosSelected(group.photos, selectedIds.value); }
function toggleBatchSelection(group: AdminPhotoGroup): void { selectedIds.value = toggleBatchPhotoSelection(group.photos, selectedIds.value); }
      ...
      <div v-else class="photo-table" role="table">
        ...
        <template v-for="group in groupedFilteredPhotos" :key="group.key">
          <div class="photo-batch-heading" :class="{ unclassified: !group.importBatch }">
            <button type="button" class="batch-toggle" :aria-expanded="isBatchExpanded(group)" :aria-label="isBatchExpanded(group) ? t('admin.collapseBatch') : t('admin.expandBatch')" @click="toggleBatchExpanded(group)"><ChevronDown :size="15" :class="{ expanded: isBatchExpanded(group) }" /><strong>{{ batchLabel(group) }}</strong></button>
            <div class="batch-heading-actions"><span>{{ t('admin.batchPhotoCount', { count: group.photos.length }) }}</span><button type="button" class="batch-select" :aria-pressed="isBatchSelected(group)" :title="isBatchSelected(group) ? t('admin.deselectBatch') : t('admin.selectBatch')" @click.stop="toggleBatchSelection(group)"><Check :size="13" /> {{ isBatchSelected(group) ? t('admin.deselectBatch') : t('admin.selectBatch') }}</button></div>
          </div>
          <template v-if="isBatchExpanded(group)"><template v-for="photo in group.photos" :key="photo.id">
...
@media (max-width: 700px) { .photo-filters { align-items: stretch; flex-direction: column; }.filter-field input, .filter-field select { min-height: 40px; min-width: 0; width: 100%; }.status-select { font-size: 10px; min-height: 38px; min-width: 92px; max-width: 100px; padding: 5px 4px; } }
.selection-toolbar { align-items: center; background: var(--surface-soft); border: 1px solid var(--line); border-radius: 6px; display: flex; flex-wrap: wrap; gap: 10px; margin: 0 0 16px; padding: 9px 11px; }.select-all { align-items: center; color: var(--muted); display: inline-flex; font-size: 11px; gap: 7px; min-height: 40px; }.selection-count { color: var(--ink); font-size: 12px; font-weight: 650; }.selection-hint { color: var(--muted); font-size: 11px; }.selection-actions { align-items: center; display: flex; flex-wrap: wrap; gap: 5px; margin-left: auto; }.tool-button { align-items: center; background: var(--ink); border-radius: 4px; color: var(--paper); display: inline-flex; font-size: 11px; gap: 5px; min-height: 38px; padding: 7px 9px; }.tool-button.subtle { background: transparent; border: 1px solid var(--line); color: var(--ink); }.tool-button.danger { background: #9b4d4d; color: #fff; }.tool-button:hover:not(:disabled) { opacity: .82; }.tool-button:disabled { cursor: wait; opacity: .4; }.clear-selection { align-items: center; background: transparent; color: var(--muted); display: inline-flex; min-height: 38px; min-width: 38px; padding: 5px; }.clear-selection:hover { color: var(--ink); }.photo-preview-overlay { align-items: center; background: rgba(8, 8, 9, .76); display: flex; inset: 0; justify-content: center; padding: 32px; position: fixed; z-index: 60; }.photo-preview { margin: 0; max-height: calc(100vh - 64px); max-width: min(920px, 92vw); text-align: center; }.photo-preview img { display: block; max-height: calc(100vh - 110px); max-width: 100%; object-fit: contain; }.photo-preview figcaption { color: rgba(255, 255, 255, .75); font-size: 11px; margin-top: 10px; }.action-notice { background: var(--ink); border-radius: 5px; bottom: calc(18px + env(safe-area-inset-bottom)); color: var(--paper); font-size: 12px; left: 50%; margin: 0; max-width: calc(100vw - 32px); padding: 9px 13px; position: fixed; text-align: center; transform: translateX(-50%); z-index: 70; }
@media (max-width: 700px) { .photo-batch-heading { align-items: flex-start; flex-wrap: wrap; gap: 7px; padding: 10px; }.batch-heading-actions { margin-left: auto; }.selection-toolbar { align-items: flex-start; }.selection-actions { margin-left: 0; width: 100%; }.photo-preview-overlay { padding: 16px; }.photo-preview { max-height: calc(100dvh - 32px); max-width: calc(100vw - 32px); }.photo-preview img { max-height: calc(100dvh - 84px); } }
</style>
