...
const navigationPhotos = computed(() => gallery.photoNavigation.length ? gallery.photoNavigation : gallery.visiblePhotos);
const previous = computed(() => { if (!photo.value) return null; const index = navigationPhotos.value.findIndex((item) => item.id === photo.value?.id); return index > 0 ? navigationPhotos.value[index - 1] : null; });
const next = computed(() => { if (!photo.value) return null; const index = navigationPhotos.value.findIndex((item) => item.id === photo.value?.id); return index >= 0 ? navigationPhotos.value[index + 1] ?? null : null; });
