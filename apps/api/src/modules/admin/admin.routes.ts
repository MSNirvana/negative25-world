...
const copyRequest = z.object({ sourcePhotoId: z.string().min(1), targetPhotoIds: z.array(z.string().min(1)).min(1).max(500), fields: z.array(z.enum(['location', 'address', 'rating'])).min(1).max(3) });
