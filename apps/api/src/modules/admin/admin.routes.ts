...
const copyRequest = z.object({ sourcePhotoId: z.string().min(1), targetPhotoIds: z.array(z.string().min(1)).min(1).max(500), fields: z.array(z.enum(['location', 'address', 'rating', 'status'])).min(1).max(4) });
const deleteRequest = z.object({ photoIds: z.array(z.string().min(1)).min(1).max(500) });
...
export function registerAdminRoutes(app: FastifyInstance, admin: AdminService, auth: AuthService, workspaces: WorkspaceService): void {
  ...
  app.post('/api/v1/admin/spaces/:slug/photos/bulk-copy', async (request) => { const slug = (request.params as { slug: string }).slug; const body = copyRequest.parse(request.body); return admin.copyPhotoFields(await actorFor(request, slug, auth, workspaces), body.sourcePhotoId, body.targetPhotoIds, body.fields); });
  app.post('/api/v1/admin/spaces/:slug/photos/bulk-delete', async (request) => { const slug = (request.params as { slug: string }).slug; const body = deleteRequest.parse(request.body); return admin.deletePhotos(await actorFor(request, slug, auth, workspaces), body.photoIds); });
  ...
  app.delete('/api/v1/admin/spaces/:slug/photos/:id', async (request) => { const params = request.params as { slug: string; id: string }; await admin.deletePhoto(await actorFor(request, params.slug, auth, workspaces), params.id); return { ok: true }; });
